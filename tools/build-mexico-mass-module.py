...
def clean_page(text: str) -> str:
    text = re.sub(r"(?<=\w)-\s*\n(?=\w)", "", text)
    text = re.sub(r"\b\d{1,3}\s+ORDINARIO DE LA MISA\b", "", text, flags=re.I)
    text = re.sub(r"\b(?:RITOS INICIALES|LITURGIA DE LA PALABRA|LITURGIA EUCARÍSTICA|RITO DE LA COMUNIÓN|RITO DE CONCLUSIÓN)\s+\d{1,3}\b", "", text, flags=re.I)
    text = re.sub(
...
def main() -> None:
    ...
    ordinary = standardize_choices(build_ordinary(reader), "es")
    source = {
...
""" % (
    ...
    js = js.replace("    ordinary,\n    officialOrderCorpus,", "    ordinary,\n    ordinaryStructure: 'paragraph-rubric-choices-v1',\n    officialOrderCorpus,")
