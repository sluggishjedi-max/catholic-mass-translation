...
function startServer() {
  ...
  const server = http.createServer((request, response) => {
    ...
    const route = decodeURIComponent(url.pathname === '/' ? '/V27.5.html' : url.pathname);
...
(async () => {
  ...
  try {
    await page.goto(`http://127.0.0.1:${server.address().port}/V27.5.html`, {
    ...
    const result = await page.evaluate(fixtures => {
      ...
      return {
        ...
        mass: {
          ...
          preface: italyMass.ordinary.find(item => item.id === '3.3 eucharist').songs.cei_complete_prefaces.content
            .reduce((total, line) => total + String(line.text_it || line.rubric_it || '').length, 0),
          forms: Object.fromEntries(Object.entries(italyMass.ordinary.find(item => item.id === '3.3 eucharist').forms)
            .map(([key, lines]) => [key, lines.reduce((total, line) => total + String(line.text_it || line.rubric_it || '').length, 0)]))
        },
        ...
        ,mexico: {
          ...
          preface: mexicoMass.ordinary.find(item => item.id === '3.3 eucharist').songs.cem_complete_prefaces.content
            .reduce((total, line) => total + String(line.text_es || line.rubric_es || '').length, 0),
          forms: Object.fromEntries(Object.entries(mexicoMass.ordinary.find(item => item.id === '3.3 eucharist').forms)
            .map(([key, lines]) => [key, lines.reduce((total, line) => total + String(line.text_es || line.rubric_es || '').length, 0)])),
        ...
        portugal: {
          ...
          preface: portugalMass.ordinary.find(item => item.id === '3.3 eucharist').songs.cep_complete_prefaces.content
            .reduce((total, line) => total + String(line.text_pt || line.rubric_pt || '').length, 0),
          forms: Object.fromEntries(Object.entries(portugalMass.ordinary.find(item => item.id === '3.3 eucharist').forms)
            .map(([key, lines]) => [key, lines.reduce((total, line) => total + String(line.text_pt || line.rubric_pt || '').length, 0)])),
    ...
    assert(result.mass.beta && result.mass.ordinaryLength === 32 && result.mass.corpusLength > 170000
      && result.mass.preface > 90000 && Object.values(result.mass.forms).every(length => length > 6500)
    ...
    assert(result.portugal.beta && result.portugal.ordinaryLength === 32 && result.portugal.corpusLength > 150000
      && result.portugal.preface > 65000 && Object.values(result.portugal.forms).every(length => length > 5000)
    ...
    assert(result.portugal.churchCount === 4395 && result.portugal.runtimeChurchCount === 4395 && result.portugal.exhaustive === true,
