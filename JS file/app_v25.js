// Order of Mass V25 application runtime. Data remains in the dedicated *_data.js files.
    // 통상문은 배포된 missa_data.js만 사용합니다.
    ...
    const APP_VERSION = 'V25-20260812';
    ...
    const liturgicalCalendarProfiles = Object.freeze({
        ...
        EN: Object.freeze({ id: 'US', countryCalendar: 'EN', epiphany: 'sunday', ascension: 'sunday', corpusChristi: 'sunday' }),
    ...
    function getLiturgicalCalendarProfile(langOrJurisdiction = state.currentLoc) {
        const key = String(langOrJurisdiction || 'LA').trim().toUpperCase();
    ...
    function getCountryCalendarOverride(date, countryLang = state.currentLoc) {
        const requested = String(countryLang || state.currentLoc || '').trim().toUpperCase();
    ...
    function defaultPrefaceHintForLiturgyInfo(info, date = null) {
        const selection = defaultPrefaceSelectionForLiturgyInfo(info, date);
        return selection.hint || selection.key || '';
    }
    ...
    function churchCountryFromLocation(value = state.currentLoc) {
        return normalizeSelectableLang(value || 'KR', 'KR');
    }

    function churchMapDefaultForLang(langCode = state.currentLoc) {
        const lang = normalizeSelectableLang(langCode || 'KR', 'KR');
        return churchMapDefaults[lang] || churchMapDefaults.KR;
    }
    ...
    function setChurchCaptionHtml(html) {
        const caption = document.getElementById('church-map-caption');
        if (caption) {
            caption.innerHTML = '';
            caption.hidden = true;
    ...
    async function enrichUsChurchPlaceDetails(place) {
        if (normalizeSelectableLang(state.currentLoc || '', '') !== 'EN') return place;
    ...
    async function refreshGpsBishopContext(lat, lon, lang = state.currentLoc) {
        const normalizedLang = normalizeSelectableLang(lang || '', '');
    ...
    function churchSearchQueriesForLocation(value = state.currentLoc) {
        const lang = normalizeSelectableLang(value || 'KR', 'KR');
        const localized = {
            KR: ['천주교 성당', '가톨릭 성당'],
        ...
        return Array.from(new Set([...(localized[lang] || localized.EN), 'Catholic church']));
    }

    function churchNearbyKeywordsForLocation(value = state.currentLoc) {
        const lang = normalizeSelectableLang(value || 'KR', 'KR');
        const localized = {
            KR: ['천주교', '가톨릭'],
    ...
    function allPrayerFieldValues(value) {
        return window.ordoPrayerDataApi.values(value);
    }

    function prayerCategoryDefinitions() {
        return window.ordoPrayerDataApi.categoryLabels;
    }
    ...
    function prayerAllCategoryLabel(categoryKey) {
        return window.ordoPrayerDataApi.categoryAliases(categoryKey);
    }
    ...
    function prayerOriginalLanguages(entry) {
        return window.ordoPrayerDataApi.originalLanguages(entry);
    }
    ...
    function syncAuxPanelsWithSettings() {
        ...
        if (activeTab === 'churches') {
            state.churchCountry = churchCountryFromLocation(state.currentLoc || 'KR');
    ...
    function showAppTab(tab) {
        ...
        if (state.activeTab === 'churches' && !state.churchCountry) state.churchCountry = churchCountryFromLocation(state.currentLoc || 'KR');
        applyActiveTabState();
    ...
    function formatCit(cit, toVN) {
        return formatBibleCit(cit, toVN);
    }
    ...
    // 전례명과 언어별 키워드에 따른 헤더 색상 결정 함수
    function getLiturgicalColor(name) {
        return colorFromSourceTitle(name, liturgyColorMap.green);
    }
    const dailyReadingPlaceholders = {
        entrance: { kr: '(오늘의 고유 입당송)', vn: '(Ca nhập lễ hôm nay...)', en: '(Entrance antiphon for today)', jp: '(今日の入祭唱)', la: '' },
        collect: { kr: '(오늘의 본기도)', vn: '(Lời nguyện nhập lễ hôm nay...)', en: '', jp: '', la: '' },
        reading1: { kr: '(오늘의 제1독서)', vn: '(Bài đọc I hôm nay...)', en: '(Reading I for today)', jp: '(今日の第一朗読)', la: '' },
        psalm: { kr: '(오늘의 화답송)', vn: '(Đáp ca hôm nay...)', en: '(Responsorial Psalm for today)', jp: '(今日の答唱詩編)', la: '' },
        reading2: { kr: '(오늘의 제2독서)', vn: '(Bài đọc II hôm nay...)', en: '(Reading II for today)', jp: '(今日の第二朗読)', la: '' },
        gospel_accl: { kr: '(오늘의 복음 환호송)', vn: '(Tung hô Tin Mừng hôm nay...)', en: '(Alleluia or verse before the Gospel)', jp: '', la: '' },
        gospel: { kr: '(오늘의 복음)', intro_vn: '', vn: '(Tin Mừng hôm nay...)', en: '(Gospel for today)', jp: '(今日の福音)', la: '' },
        prayer_offerings: { kr: '(오늘의 예물 기도)', vn: '(Lời nguyện tiến lễ hôm nay...)', en: '', jp: '', la: '' },
        communion: { kr: '(오늘의 영성체송)', vn: '(Ca hiệp lễ hôm nay...)', en: '', jp: '', la: '' },
        prayer_after: { kr: '(오늘의 영성체 후 기도)', vn: '(Lời nguyện hiệp lễ hôm nay...)', en: '', jp: '', la: '' }
    };
...
Lạy Chúa, chúng con vừa lãnh nhận hồng ân Chúa ban, xin cho chúng con mỗi khi cử hành bí tích này được hưởng dồi dào ơn cứu độ. Chúng con cầu xin…`
    ...
    function removeSummaryLines(lines) {
        return lines.filter(line => !/^["“_].+["”_]\s*(\([^)]+\))?\.?$/i.test(line));
    }
    ...
    function splitOptionsByMarker(lines, markerRegex) {
        ...
        splitSourceBlocks(lines).forEach(line => {
            ...
            if (markerRegex.test(line) || markerRegex.test(markerText) || /^(또는|Hoặc|Or|または|又は)\s*:?\s*$/i.test(markerText)) {
                if (current.length) options.push(current);
                current = [];
            } else {
                current.push(line);
    ...
    function formatVietnameseSection(key, lines) {
        ...
        cleaned.forEach(line => {
            if (/^Hoặc\b|^Hoặc đọc/i.test(line)) {
                if (current.length) options.push(current);
                current = [];
            } else {
                current.push(line);
        ...
        if (current.length) options.push(current);
        const formattedOptions = (options.length ? options : [cleaned])
    ...
    function cleanVietnameseReadingLines(lines) {
        return removeSummaryLines(lines)
            .filter(line => !/^Trích\b/i.test(line))
            .filter(line => !/^Tin Mừng Chúa Giêsu Kitô/i.test(line))
            .filter(line => !/^[ĐÐ]ó là lời Chúa\.?$/i.test(line));
    }
    ...
    function extractSections(lines, lang) {
        const raw = extractRawSections(lines, lang);
        const result = {};
        Object.keys(raw).forEach(key => { result[key] = compactSection(raw[key]); });
        return result;
    }
    ...
    async function resolveVietnameseDailyUrl(date) {
    ...
    const strictTerminalSectionPatterns = {
        ...
        EN: [/^Preface/i, /^Reflection/i, /^Meditation/i, /^Universal Prayer/i, /^Daily Reflection/i],
    ...
    function strictParsePrayerOrAntiphon(lang, key, section) {
        ...
        strictGroupAlternativeBlocks(blocks).forEach((group, index) => {
            ...
            const citationResult = ['entrance', 'communion'].includes(key) ? strictExtractCitation(section, group, lang, { preserveParentheses: true, preferBlockCitation: true }) : { citation: '', blocks: group };
            if (citationResult.citation) optionCits.push({ [`cit_${lang.toLowerCase()}`]: citationResult.citation });
    ...
    function dailySourceStorageKey(lang, date) {
        const sourceVariant = lang === 'VN' ? `:${normalizeVietnameseReadingSource(state.vnReadingSource)}` : '';
    ...
    function koreanMissalForeignProperEntry(date) {
        ...
        if (!getCountryCalendarOverride(date, leftLang)) return null;
        return koreanMissalForeignProperData[`${leftLang}:${calendarDateKey(date)}`] || null;
    }
    ...
    function citationStartForCompare(value) {
        return citationStartsForCompare(value)[0] || '';
    }
    ...
    function applyDetectedLocation(nextLoc, detectedTimeZone = '') {
        ...
        state.currentLoc = normalizeSelectableLang(nextLoc, 'KR');
        state.gpsTimeZone = detectedTimeZone || liturgicalTimeZoneForLang(state.currentLoc);
        state.liturgicalDateContext = null;
    ...
    function gpsLanguageForTimeZone(timeZone) {
        ...
        if (browserZone.startsWith('america/')) return 'EN';
        if (browserZone.includes('rome') || browserZone.includes('vatican')) return 'LA';
        return 'KR';
    }

    function gpsLocationForCoordinates(lat, lon) {
        if (lat >= 41.88 && lat <= 41.93 && lon >= 12.43 && lon <= 12.48) return 'LA';
        ...
        if (lat > 18 && lat < 72 && lon > -172 && lon < -66) return 'EN';
        return gpsLanguageForTimeZone(Intl.DateTimeFormat().resolvedOptions().timeZone || '');
    }

    function gpsTimeZoneForCoordinates(lat, lon, lang) {
        if (lang === 'KR') return 'Asia/Seoul';
        if (lang === 'VN') return 'Asia/Ho_Chi_Minh';
        if (lang === 'JP') return 'Asia/Tokyo';
        if (lang === 'LA') return 'Europe/Rome';
        if (lang === 'EN') {
        ...
        return Intl.DateTimeFormat().resolvedOptions().timeZone || liturgicalTimeZoneForLang(lang);
    }
    ...
    const localizedCountryNames = Object.freeze({
        KR: { KR: '한국', VN: '베트남', US: '미국', JP: '일본', VA: '바티칸' },
        VN: { KR: 'Hàn Quốc', VN: 'Việt Nam', US: 'Hoa Kỳ', JP: 'Nhật Bản', VA: 'Vatican' },
        EN: { KR: 'South Korea', VN: 'Vietnam', US: 'United States', JP: 'Japan', VA: 'Vatican City' },
        JP: { KR: '韓国', VN: 'ベトナム', US: 'アメリカ', JP: '日本', VA: 'バチカン' },
        LA: { KR: 'Corea Meridiana', VN: 'Vietnamia', US: 'Civitates Foederatae', JP: 'Iaponia', VA: 'Civitas Vaticana' }
    });
    ...
    function syncLocalizedChromeAndSettings() {
        ...
        if (locationSelect) {
            Array.from(locationSelect.options).forEach(option => {
                ...
                option.textContent = `${country} / ${localizedLanguageOptionLabel(location.lang, ui)}`;
            });
        }
    ...
    function localizedLiturgyLoadingMessageForLangs(langCodes, uiLang = state.uiLang) {
        return Array.from(new Set((langCodes || []).map(lang => normalizeSelectableLang(lang, '')).filter(Boolean)))
            .map(lang => localizedLiturgyLoadingMessageForLang(lang, uiLang))
            .join('\n');
    }
    ...
    function getEucharistInlineOptionBlock(item, optionKey) {
        const formLines = item && item.forms && Array.isArray(item.forms['3']) ? item.forms['3'] : [];
        return formLines.find(line =>
            line && line.type === 'optionBlock' && line.optionKey === optionKey && line.options
        ) || null;
    }
    ...
    function eucharistSongTitleBlob(songs, key) {
        const entry = songs[key] || {};
        const title = entry.title && typeof entry.title === 'object'
            ? Object.values(entry.title).join(' ')
            : (entry.title || '');
        return `${key} ${title}`.normalize('NFKC');
    }

    function eucharistPrefaceCategoryIndex(blob) {
        const text = String(blob || '');
        const tests = [
            [/공통|Common Preface|Praefatio Communis|Kinh Tiền Tụng Chung|共通/i, 0],
            [/연중\s*주일|Sundays in Ordinary Time|Dominicis per Annum|Chúa Nhật Thường Niên|年間主日/i, 1],
            [/대림|Advent|Mùa Vọng|待降/i, 2],
            [/성탄|Nativity|Giáng Sinh|Nativitate|降誕/i, 3],
            [/사순|Lent|Mùa Chay|Quadragesim|四旬/i, 4],
            [/부활|Easter|Phục Sinh|Pasch|復活/i, 5],
            [/성모|마리아|Mary|Maria|Đức Trinh Nữ Maria|Đức Mẹ|Beata Maria|聖母|マリア|Guadalupe|Immaculate|Assumption|Annunciation|お告げ/i, 6],
            [/성인|모든 성인|All Saints|Holy Men and Women|Sanctis|Các Thánh Nam Nữ|諸聖人|요셉|Joseph|Giuse|ヨセフ|세례자 요한|John the Baptist|Gioan Tẩy Giả|洗礼者/i, 7],
            [/사도|Apostle|Apostolis|Tông Đồ|使徒|베드로.*바오로|Peter.*Paul|Phêrô.*Phaolô|ペトロ.*パウロ/i, 8],
            [/순교|Martyr|Tử Đạo|殉教/i, 9],
            [/목자|Pastor|Mục Tử|牧者/i, 10],
            [/수도자|동정녀|Virgin|Religious|Đồng Trinh|Tu Sĩ|Virginibus|Religiosis|おとめ|修道/i, 11],
            [/위령|Dead|Defunct|Qua Đời|死者/i, 12],
            [/천사|Angel|Thiên Thần|Angelis|天使/i, 13],
            [/감사\s*기도\s*제?2\s*양식\s*고유|Eucharistic Prayer II|Praefatio Eucharistica II|第2奉献文/i, 15]
        ];
        const matched = tests.find(([pattern]) => pattern.test(text));
        return matched ? matched[1] : 14;
    }

    function eucharistPrefaceSortNumber(blob) {
        const text = String(blob || '');
        const arabic = text.match(/(?:감사송|Preface|Praefatio|叙唱|Tụng)\D{0,28}(\d{1,2})/i)
            || text.match(/\b(\d{1,2})\b/);
        if (arabic) return Number.parseInt(arabic[1], 10);
        const roman = text.match(/\b([IVX]{1,5})\b(?!\s*[A-Za-z])/i);
        return roman ? romanToNumber(roman[1]) : Number.MAX_SAFE_INTEGER;
    }
