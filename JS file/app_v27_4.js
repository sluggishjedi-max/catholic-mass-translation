// Order of Mass V27.4 application runtime. Liturgical data is loaded from country-owned modules.
    ...
    function mergeIndependentOrdinary(base, jurisdiction) {
        if (Array.isArray(base) || Array.isArray(jurisdiction)) {
            ...
            const right = Array.isArray(jurisdiction) ? jurisdiction : [];
            return Array.from({ length: Math.max(left.length, right.length) }, (_, index) => (
                mergeIndependentOrdinary(left[index], right[index])
            ));
        }
        const leftObject = base && typeof base === 'object';
        const rightObject = jurisdiction && typeof jurisdiction === 'object';
        if (leftObject || rightObject) {
            const merged = Object.assign({}, leftObject ? base : {});
            Object.entries(rightObject ? jurisdiction : {}).forEach(([key, value]) => {
                merged[key] = key in merged ? mergeIndependentOrdinary(merged[key], value) : cloneData(value);
            });
            return merged;
        }
        return jurisdiction === undefined ? base : jurisdiction;
    }
    ...
    const liturgicalCalendarProfiles = Object.freeze({
        ...
        'GB-ENG': Object.freeze({ id: 'ENGLAND', countryCalendar: 'GB-ENG', epiphany: 'british', ascension: 'thursday', corpusChristi: 'sunday' }),
        'GB-WLS': Object.freeze({ id: 'WALES', countryCalendar: 'GB-WLS', epiphany: 'british', ascension: 'thursday', corpusChristi: 'sunday' }),
        'GB-SCT': Object.freeze({ id: 'SCOTLAND', countryCalendar: 'GB-SCT', epiphany: 'sunday', ascension: 'thursday', corpusChristi: 'sunday' }),
        PH: Object.freeze({ id: 'PHILIPPINES', countryCalendar: 'PH', epiphany: 'sunday', ascension: 'sunday', corpusChristi: 'sunday' }),
        AU: Object.freeze({ id: 'AUSTRALIA', countryCalendar: 'AU', epiphany: 'sunday', ascension: 'sunday', corpusChristi: 'sunday' }),
        NZ: Object.freeze({ id: 'NEW_ZEALAND', countryCalendar: 'NZ', epiphany: 'sunday', ascension: 'sunday', corpusChristi: 'sunday' }),
    ...
    function countryMassProperEntriesForDate(jurisdiction, date) {
        ...
        const dynamic = typeof module.dynamicCalendar === 'function'
            ? module.dynamicCalendar(date, { easter: computeEasterSunday(date.getFullYear()) })
            : [];
        const fixedEntries = Array.isArray(fixed) ? fixed : (fixed ? [fixed] : []);
        const applicableFixedEntries = fixedEntries.filter(entry => fixedCalendarEntryApplies(date, saintEntry(
            entry.color || 'white',
            entry.names || { EN: entry.title },
            { rank: entry.rank || 'optional' }
        )));
        return [
            ...applicableFixedEntries,
    ...
    const countryFixedSaintsCalendar = {
        ...
        'GB-ENG': countryMassCalendarAsSaintEntries('GB-ENG'),
        'GB-WLS': countryMassCalendarAsSaintEntries('GB-WLS'),
        'GB-SCT': countryMassCalendarAsSaintEntries('GB-SCT'),
        PH: countryMassCalendarAsSaintEntries('PH'),
        AU: countryMassCalendarAsSaintEntries('AU'),
        NZ: countryMassCalendarAsSaintEntries('NZ'),
        JP: {
    ...
    function getCountryCalendarOverride(date, countryLang = state.selectedLocationCode || state.currentLoc) {
        ...
        const dynamicEntries = countryModule && typeof countryModule.dynamicCalendar === 'function'
            ? countryModule.dynamicCalendar(date, { easter: computeEasterSunday(date.getFullYear()) })
            : [];
        const dynamicEntry = Array.isArray(dynamicEntries) ? dynamicEntries[0] : dynamicEntries;
        if (dynamicEntry) {
            if (dynamicEntry.suppressUniversalFixed) {
                return generatedSeasonalCalendarOverride(date, { includeFixed: false });
            }
    ...
    function getEpiphanyObserved(year) {
        ...
        if (profile.epiphany === 'fixed') return nominal;
        if (profile.epiphany === 'british') {
            if (nominal.getDay() === 6) return addDays(nominal, 1);
            if (nominal.getDay() === 1) return addDays(nominal, -1);
            return nominal;
        }
        return firstSundayBetween(year, 0, 2, 8);
    }

    function getBaptismOfLord(year) {
        const epiphany = getEpiphanyObserved(year);
        const profile = getLiturgicalCalendarProfile();
        if (['sunday', 'british'].includes(profile.epiphany) && epiphany.getDay() === 0 && epiphany.getDate() >= 7) return addDays(epiphany, 1);
    ...
    function getMovableCelebration(date) {
        ...
        const secondSundayCandidate = ['fixed', 'british'].includes(profile.epiphany) ? getSecondSundayAfterChristmas(y) : null;
        const secondSundayAfterChristmas = secondSundayCandidate && !sameDay(secondSundayCandidate, epiphany)
            ? secondSundayCandidate
            : null;
        const movable = [
    ...
    const locationMeta = {
        ...
        'GB-ENG': { lang: 'EN', label: '잉글랜드 / English (Beta)', timeZone: 'Europe/London', beta: true, dataJurisdiction: 'GB-ENG' },
    ...
    globalThis.rebuildCountryChurchDirectory = rebuildCountryChurchDirectory;
    ...
    const churchMapDefaults = {
        ...
        'GB-ENG': { center: { lat: 51.5074, lng: -0.1278 }, zoom: 11, query: 'Catholic church London England' },
    ...
    const auxUiText = {
        ...
        ZH: {
            ...
            hymnWarning: '各語言聖歌仍在整理中，部分歌詞可能無法正常顯示。',
            hymnCategoryAll: '全部',
            hymnEmpty: '找不到聖歌。連接已上傳的聖歌檔案後，可在此搜尋編號、標題與歌詞。',
            hymnBodyPlaceholder: '連接已上傳的檔案後，歌詞會顯示在這裡。',
            churchTitle: '附近聖堂',
            churchNote: '請在下方搜尋聖堂名稱以顯示位置；啟用 GPS 後，將顯示附近的聖堂。',
            churchPlaceholder: '搜尋聖堂名稱',
            churchReady: '',
            churchFallback: '',
            search: '搜尋'
        }
    ...
    function renderAuxWarning(id, field, languageCodes) {
        ...
        if (!element) return;
        const languages = Array.from(new Set((languageCodes || [])
            .map(lang => normalizeSelectableLang(lang, ''))
            .filter(lang => lang && auxUiText[lang] && auxUiText[lang][field])));
        element.innerHTML = languages.map(lang => [
            '<div class="aux-warning-line">',
            `<span class="aux-warning-language">${escapeHtml(auxWarningLanguageLabels[lang] || lang)} :</span>`,
            `<span class="aux-warning-text">${escapeHtml(auxUiText[lang][field])}</span>`,
            '</div>'
        ].join('')).join('');
    }
    ...
    function localizeAuxPanels() {
        ...
        renderAuxWarning('hymn-dev-warning', 'hymnWarning', ['KR', hymnLanguage]);
    ...
    function churchSearchQueriesForLocation(value = state.selectedLocationCode || state.currentLoc) {
        ...
        const regional = {
            ...
            'GB-ENG': ['Catholic church England', 'Catholic parish England'],
    ...
    function syncLocationControlForGps() {
        const row = document.getElementById('manual-loc-row');
        const select = document.getElementById('set-loc');
        ...
        Array.from(select.options).forEach(option => {
            option.hidden = state.useGps ? option.value !== selectedCode : option.value === 'INTL';
        });
        Array.from(select.querySelectorAll('optgroup')).forEach(group => {
            group.hidden = Array.from(group.querySelectorAll('option')).every(option => option.hidden);
        });
        row.classList.toggle('gps-location-locked', !!state.useGps);
    }
...
Lạy Chúa, chúng con vừa lãnh nhận hồng ân Chúa ban, xin cho chúng con mỗi khi cử hành bí tích này được hưởng dồi dào ơn cứu độ. Chúng con cầu xin…`
    ...
    function traditionalChineseStaticDate(value) {
    ...
    function traditionalChineseStaticCycleSignature(date) {
        const info = buildGeneratedLiturgyInfo(date);
        const meta = (info && info.meta) || getSeasonMeta(date);
        const englishName = normalizeCalendarNameForCompare(info && info.names && info.names.EN);
        if (meta.special || meta.specialMassKey) {
            return `special:${meta.specialMassKey || englishName || calendarDateKey(date)}`;
        }
        const cycle = date.getDay() === 0 ? getSundayCycle(date) : getWeekdayCycle(date);
        return `${date.getDay() === 0 ? 'sunday' : 'weekday'}:${cycle}:${meta.season}:${meta.week}:${meta.day}`;
    }

    function traditionalChineseStaticRecordCandidates(date) {
        const data = traditionalChineseStaticMassData();
        if (!data) return [];
        const exactIds = data.byDate && data.byDate[formatDateYmd(date)];
        if (Array.isArray(exactIds) && exactIds.length) {
            return exactIds.map(id => data.records && data.records[id]).filter(Boolean);
        }

        const signature = traditionalChineseStaticCycleSignature(date);
        const meta = getSeasonMeta(date);
        const cycleIds = date.getDay() === 0
            ? data.cycles && data.cycles.sundays && data.cycles.sundays[meta.sundayCycle || getSundayCycle(date)]
            : data.cycles && data.cycles.weekdays && data.cycles.weekdays[meta.weekdayCycle || getWeekdayCycle(date)];
        const preferred = (Array.isArray(cycleIds) ? cycleIds : [])
            .map(id => data.records && data.records[id])
            .filter(record => record && record.canonicalDate)
            .filter(record => {
                const canonicalDate = traditionalChineseStaticDate(record.canonicalDate);
                return canonicalDate && traditionalChineseStaticCycleSignature(canonicalDate) === signature;
            });
        if (preferred.length) return preferred;

        // Solemnities and feasts have cycle-independent propers and can be
        // represented by a canonical PDF that is filed under another cycle.
        if (signature.startsWith('special:')) {
            return Object.values(data.records || {}).filter(record => {
                const canonicalDate = traditionalChineseStaticDate(record && record.canonicalDate);
                return canonicalDate && traditionalChineseStaticCycleSignature(canonicalDate) === signature;
            });
        }
        return [];
    ...
    function traditionalChineseStaticMassRecord(date) {
        const entries = traditionalChineseStaticRecordCandidates(date);
        ...
        const day = entries.find(entry => strictIsDayMassLabel(entry.title));
        if (day) return day;
        return entries.find(entry => !strictIsVigilLabel(entry.title) && !strictIsNightMassLabel(entry.title)) || entries[0];
    }

    function fetchTraditionalChineseDailyStaticSource(date) {
        const record = traditionalChineseStaticMassRecord(date);
        if (!record || !record.text) throw new Error('CRBC static daily Mass text is not available for this cycle/date.');
        return [
            `Title: ${record.title || ''}`,
            ...
            if (fallbackUrl && fallbackUrl !== entryUrl) {
                try {
                    ...
                    if (['reading1', 'psalm', 'gospel'].every(key => fallbackParsed.data && fallbackParsed.data[key])) {
                        ...
                        selectedLink = fallbackLink;
                    }
                } catch (error) {
                    console.warn(`Country daily fallback source failed: ${fallbackUrl}`, error);
                }
            }
        }
        if (lang === 'EN' && !['reading1', 'psalm', 'gospel'].every(key => parsed.data && parsed.data[key])) {
        ...
        if (lang === 'EN') {
            ...
            if (properSource && typeof properSource.url === 'function') {
                ...
                try {
                    ...
                    if (!parsed.title && properParsed.title) parsed.title = properParsed.title;
                } catch (error) {
                    console.warn(`Country Mass proper source failed: ${properUrl}`, error);
                }
            }
        }
        const fixedTitle = strictFixedDailyTitle(lang, date);
    ...
    function localMissalEntryHasProperText(entry) {
        return Boolean(entry && entry.data && Array.from(localMissalPrayerSectionIds)
            .some(key => cleanNodeText(entry.data[key])));
    }
    ...
    function bestLocalMissalEntry(entries, currentTitle, requireProperText = true) {
        let best = null;
        (entries || []).forEach((entry, index) => {
            if (!entry || (requireProperText && !localMissalEntryHasProperText(entry))) return;
    ...
    function localMissalCommonPageForCountryEntry(entry) {
        const title = normalizeSemanticText(entry && entry.title || '');
        if (!title) return null;
        if (/blessed virgin|our lady|virgin mary|immaculate conception|assumption/.test(title)) return 1039;
        if (/martyrs|companions/.test(title)) return 1051;
        if (/martyr/.test(title)) return 1058;
        if (/pope|bishop/.test(title)) return 1070;
        if (/priest/.test(title)) return 1076;
        if (/missionary/.test(title)) return 1083;
        if (/doctor of the church/.test(title)) return 1086;
        if (/virgin/.test(title)) return 1090;
        if (/abbot|abbess|religious/.test(title)) return 1105;
        if (/\bsaints?\b|\bblessed\b/.test(title)) return 1097;
        return null;
    }

    function localMissalComposeCountryProper(entry, languageData, date, currentTitle) {
        if (!entry || !languageData) return localMissalEntryHasProperText(entry) ? entry : null;
        ...
        let base = dated && dated.score >= 0.72 ? dated.entry : null;
        if (!base && catalog && catalog.score >= 0.72) base = catalog.entry;
        const commonPage = localMissalCommonPageForCountryEntry(entry);
        const common = commonPage == null ? null : (languageData.catalog || [])
            .find(candidate => candidate && candidate.kind === 'common' && Number(candidate.page) === commonPage) || null;
        const data = Object.assign({}, common && common.data || {}, base && base.data || {}, entry.data || {});
        if (!Array.from(localMissalPrayerSectionIds).some(key => cleanNodeText(data[key]))) return null;
        return Object.assign({}, common || {}, base || {}, entry, { data });
    }
    ...
    function localMissalEntryForLanguage(lang, date) {
        ...
        if (code === 'EN') {
            ...
            const dated = bestLocalMissalEntry(datedEntries, currentTitle, false);
            if (dated && (dated.score >= 0.52 || datedEntries.length === 1)) {
                const composed = localMissalComposeCountryProper(
                    dated.entry,
                    localMissalLanguageData(code),
                    date,
                    currentTitle
                );
                if (composed) return composed;
                // A matched national observance with no published proper must not
                // fall through to an unrelated universal celebration on the same date.
                return Object.assign({}, dated.entry, { data: Object.freeze({}) });
            }
        }
    ...
    function gpsLanguageForTimeZone(timeZone) {
        ...
        if (browserZone.includes('sydney') || browserZone.includes('melbourne') || browserZone.includes('brisbane')
            || browserZone.includes('adelaide') || browserZone.includes('perth') || browserZone.includes('darwin')
            || browserZone.includes('hobart') || browserZone.includes('broken_hill')) return 'AU';
        if (browserZone.includes('auckland') || browserZone.includes('wellington') || browserZone.includes('chatham')) return 'NZ';
    ...
    function gpsLocationForCoordinates(lat, lon) {
        ...
        if (lat > 51.3 && lat < 53.5 && lon > -5.5 && lon < -2.65) return 'GB-WLS';
        if (lat > 49.8 && lat < 55.9 && lon > -6.5 && lon < 2.1) return 'GB-ENG';
        if (lat > 18 && lat < 72 && lon > -172 && lon < -66) return 'US';
        return Number.isFinite(lat) && Number.isFinite(lon)
            ? 'INTL'
            : gpsLanguageForTimeZone(Intl.DateTimeFormat().resolvedOptions().timeZone || '');
    }
    ...
    function simulateGPS() {
        ...
        if (!navigator.geolocation) {
            ...
            if (applyDetectedLocation(gpsLanguageForTimeZone(fallbackZone), fallbackZone)) fetchMassData();
            return;
        }
    ...
    const noticeUiText = Object.freeze({
        KR: { title: '사용 전 안내', accept: '동의합니다', decline: '동의하지 않고 종료', closed: '동의하지 않아 이용을 종료합니다.', dateLimit: '오늘 전후 7일을 초과하는 전례는 조회할 수 없습니다.' },
    ...
    function shouldSuppressAIFallbackForLine(line, baseId, lower) {
        ...
        // A source-choice row intentionally clears the non-selected official
    ...
    function normalizedReadingBodyText(value, lower) {
        const parts = String(value || '')
            .replace(/<br\s*\/?>/giu, '\n')
            .split(/\n+/)
            .map(part => part.trim())
            .filter(Boolean);
        const joined = ['zh', 'jp'].includes(lower) ? parts.join('') : parts.join(' ');
        return joined
            .replace(['zh', 'jp'].includes(lower) ? /\s+/gu : /\s+/g, ['zh', 'jp'].includes(lower) ? '' : ' ')
            .trim();
    }
    ...
    function alignReadingTextToKoreanParagraphs(value, lower, koreanParagraphs) {
        const count = koreanParagraphs.length;
        if (!count) return [];
        if (lower === 'kr') return koreanParagraphs.slice();
        const text = normalizedReadingBodyText(value, lower);
        if (!text) return Array(count).fill('');
        if (count === 1) return [text];
        const koreanLengths = koreanParagraphs.map(paragraph => Math.max(1, cleanNodeText(paragraph).length));
        const koreanTotal = koreanLengths.reduce((sum, length) => sum + length, 0);
        const candidates = readingBreakCandidates(text, lower);
        const boundaries = [];
        let koreanProgress = 0;
        let previous = 0;
        for (let index = 0; index < count - 1; index += 1) {
            koreanProgress += koreanLengths[index];
            const desired = Math.max(previous + 1, Math.min(text.length - 1, Math.round(text.length * koreanProgress / koreanTotal)));
            const viable = candidates.filter(candidate => candidate.index > previous && candidate.index < text.length);
            const arbitraryPenalty = Math.max(2, Math.round(text.length * 0.08));
            const chosen = viable.concat({ index: desired, penalty: 2 })
                .sort((left, right) => {
                    const leftScore = Math.abs(left.index - desired) + left.penalty * arbitraryPenalty;
                    const rightScore = Math.abs(right.index - desired) + right.penalty * arbitraryPenalty;
                    return leftScore - rightScore || left.index - right.index;
                })[0];
            let boundary = chosen ? chosen.index : desired;
            if (boundary <= previous) boundary = Math.min(text.length - 1, previous + 1);
    ...
    // Reading sources use different physical wrapping rules (especially PDF
    // sources). At display time, keep each language's own punctuation and word
    // boundaries while placing its successive phrases in the same rows as the
    // Korean source. The next paragraph therefore starts at the same vertical
    // position in both columns even when either phrase wraps on screen.
    function splitDailyReadingBodyRowsByKorean(lines, baseId) {
        if (!strictReadingKeys.has(baseId) || !Array.isArray(lines)) return lines || [];
        const lowers = SUPPORTED_LANGS.map(lang => lang.toLowerCase());
        return lines.flatMap(line => {
            const koreanText = String(line && line.text_kr || '');
            const koreanParagraphs = koreanText.split(/\n+/).map(part => part.trim()).filter(Boolean);
            if (line && line.role_kr !== 'body' || koreanParagraphs.length < 2) return [line];
            const textSegments = Object.fromEntries(lowers.map(lower => [
                lower,
                alignReadingTextToKoreanParagraphs(line[`text_${lower}`], lower, koreanParagraphs)
            ]));
            const aiSegments = Object.fromEntries(lowers.map(lower => [
                lower,
                alignReadingTextToKoreanParagraphs(line[`text_${lower}_ai`], lower, koreanParagraphs)
            ]));
            return koreanParagraphs.map((_, paragraphIndex) => {
                const row = Object.assign({}, line);
                lowers.forEach(lower => {
                    row[`sp_${lower}`] = paragraphIndex === 0 ? (line[`sp_${lower}`] || '') : '';
                    row[`text_${lower}`] = textSegments[lower][paragraphIndex] || '';
                    row[`text_${lower}_ai`] = aiSegments[lower][paragraphIndex] || '';
                    row[`role_${lower}`] = row[`text_${lower}`] || row[`text_${lower}_ai`] ? (line[`role_${lower}`] || 'body') : '';
                    row[`rubric_${lower}`] = paragraphIndex === 0 ? (line[`rubric_${lower}`] || '') : '';
                    row[`verse_refs_${lower}`] = paragraphIndex === 0 ? (line[`verse_refs_${lower}`] || []) : [];
                });
                return row;
            });
        });
